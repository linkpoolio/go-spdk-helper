...
// controllers see only an ungraceful transport drop when the target process
// dies and sit in a "failover already in progress" state until
// ctrlr_loss_timeout, which can wedge their reactor.
//
// Intended to be called from the target's shutdown path (e.g. a Kubernetes
// PreStop hook) right before the SPDK target process is killed.
...
// spdk_tgt cannot burn the whole shutdown window.
func NvmfSubsystemDrainListenersCmd() cli.Command {
	return cli.Command{
		Name:  "drain-listeners",
		Usage: "remove every listener from every active NVMe-oF subsystem so connected initiators see a clean disconnect; intended for shutdown (e.g. PreStop) hooks",
		Flags: []cli.Flag{
			cli.DurationFlag{
				Name:  "timeout",
